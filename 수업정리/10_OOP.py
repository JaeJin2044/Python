...
'''
    [Object Oriented Programming] - 객체지향 프로그래밍
        ...
        - '클래스'와 '객체'
            ...
            2. 클래스(class)
                - 정의 : 객체를 정의해 놓은 것 (어떠한 객체를 만들 것인지)
                - 용도 : 객체를 생성
        ...
        - 객체의 구성 요소 : 속성, 기능
            (속성 = 변수, 기능 = 함수)
            (속성 : 갤럭시s20의 색상 정보, 기능 : 갤럭시s20으로 사진을 찍는다.)
...
'''
    1. 클래스(설계도/틀)는 속성을 정의하거나 기능을 정의할 수 있다.
        > 함수와 마찬가지로 클래스도 정의해놓기만 하면 프로그램 수행에 영향이 없다.
        > 객체(인스턴스)를 생성한 뒤부터 클래스에 작성된 코드의 효력이 발생할 수 있다.

    2. 클래스 안에 정의된 함수는 '메서드(method)'라고 부른다.
        > 메서드를 만들 때는 반드시 최소 하나의 매개변수가 필요하다.
        > 자 자기 자신을 의미하는 self라는 이름으로 한다. (꼭 self가 아니어도 된다)
'''


# class 클래스명:  --> 클래스 정의

class Car :         # self --> 매개변수로 동작하는게 아니라 인스턴스로 동작한다.
    def power_on(self): # self 매개변수는 무조건 포함시켜야 한다. //필요 없더라도 무조건 포함
        print("부릉부릉!") # self에는 bmw(이 메서드를 호출한 인스턴스)가 대입
        self.power = True  # bmw.power = True 
        # bmw 인스턴스에 power라는 변수가 추가 됐다. (변수 생성 코드) 
        self.drive()    # bmw.drvie()
...
# 문자열 관련 함수 --> 문자열.함수(), 리스트 관련 함수 --> 리스트.함수()
...
tico.drive()
tico.power = "시동 켜짐"   # 클래스 코드 바깥에서 변수를 생성
print("tico의 시동 여부 : ", tico.power)

tico.model = "TICO의 최신 모델"
# tico는 model 변수를 추가

print(tico.model)
print()

# print(bmw.model) # bmw 는 model이라는 변수가 없다.

# 만들어지는 인스턴스별로 속성이 다를 수 있다.
...
car1_max_speed = 200 # 자동차의 최고속도 변수
...
def drive_car(model, power, max_speed, speed) : # speed : 주행하고 싶은 속도 
    ...
    if speed > max_speed :
        print("{}의 최고 속도는 {}km입니다. 속도를 줄입니다.".format(model, max_speed))
        speed = max_speed

    print("{}km로 주행합니다. 출발 ~~".format(speed))

drive_car(car1_model, car1_power, car1_max_speed, 200) # 자동차1을 200으로 주행
#            "BMW"     "False"       200         200
print()
...
class Car:
    def drive_car(self,speed) : # speed : 주행하고 싶은 속도 
        ...
        if speed > self.max_speed :
            print("{}의 최고 속도는 {}km입니다. 속도를 줄입니다.".format(self.model, self.max_speed))
...
car1.power = False
car1.max_speed = 200
# 인스턴스만 대입되면 바깥의 변수들 다 안으로 대입 가능 하다..
...
print()
car2.drive_car(200)
# drive_Car를 호출 시 정의된 매개변수는 2개(self, speed)이다.
# self에는 자동으로 호출하는 인스턴스가 대입
# 그 뒤 매개변수부터는 호출할 때 값을 전달해야 한다.

# 1) 인스턴스라는 하나의 변수에 모든 속성들을 담아놨다. (연관성이 생김)
# 2) 함수를 호출할 때 그냥 호출해도 self에는 호출한 인스턴스가 대입되기 때문에
#    함수의 수행문 안에 있는 self.~~가 호출한 인스턴스의 속성들을 사용할 수 있다.


# 3. 클래스의 사용 (2) - 생성자
#   1) 인스턴스 생성시 자동으로 호출되는 메서드(함수) --> 무조건 호출된다.
#   2) 목적 : 인스턴스 생성과 동시에 속성을 추가/초기값 지정을 하고 싶을때 사용(초기화)
#   3) 생성자 함수의 이름(규칙) :  __init__  -->  _(언더바 앞뒤로 2개씩)  // 무조건이걸로 사용해야한다 !! 규칙임
...
class Car:
    def __init__(self, model):
        print("생성자 호출!")
        self.model = model  # self(자기 자신의 인스턴스)에 model 속성 추가 

car1 = Car("BMW")  # 인스턴스를 생성하는 코드
#    원래 생성자함수를 호출하는 코드
#    생성자 함수가 호출되고 나서 인스턴스가 만들어진다.
#    생성자 함수가 없으면 호출되지 않고 만들어 진다.

print("car1의 모델 :", car1.model)

car2 = Car("SONATA")
print("car2의 모델 : ", car2.model)

# 생성자 함수를 이용하면, 모든 인스턴스가 공통적으로 지녀야할 속성을 만들 수 있다.
# 인스턴스 생성할 때 무조건 호출되는 생성자 함수라서
print()
...
class Car:
    def __init__(self,model,power,max_speed):
        ...
        self.model = model # 내 인스턴스에 model속성을 생성하며 매개변수로 받을 model값을 대입
        self.power = power
...
'''
    ...
    * 클래스변수는 한 번 만들어지면 '클래스명'이나 '인스턴스명으로 접근(사용)할 수 있다.
       인스턴스변수는 인스턴스별로 독립적으로 생성된다.
...
class Person:
    ...
    def __print_info(self): # private
        print("{}님은 {}에 거주합니다.".format(self.name, self.__addr))

    def print_info2(self): # 다른 public 메서드를 통한 private 메서드 우회 접근 
        print("print_info2")
        self.__print_info()

lee = Person()
print(lee.name) # 인스턴스를 통한 클래스변수 사용
#print(lee.__addr) # 클래스 바깥 코드에서 접근할 수 없는 private 변수
lee.print_info()
#lee.__print_info()
lee.print_info2()
print()
...
'''
    추상적이다!
        --> 동물이라는 클래스 정의하며 동물은 '운다'라는 추상적인 개념만 정의해놓고
            각 동물들이 실제로 어떻게 우는지는 각자 정의하도록(자식클래스) 하는 것

        --> 이때 운다 라는 메서드를 추상메서드라고 한다.
            추상메서드는 자식클래스에서 '반드시' 정의해야 한다.
...
class Dog(Animal):
    def Cry(self):          # 반드시 똑같은 메서드명 적어줘야한다 .. 수행문은 다를지언정 
        print("멍멍")
